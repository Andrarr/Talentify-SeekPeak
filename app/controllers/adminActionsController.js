...
export const updateRole = async (req, res) => {
  ...
  if (req.params.id && req.body.role && req.body.name) {
    ...
    if (req.body.name != user.name) {
      return res.send({
        message: "Name and id of user do not match to an existing user!",
      });
    }
    if (!user) {
      ...
      return res.send({
        message: `'${user.name}' role has been updated to '${req.body.role}'!`,
      });
    }
  }
};
...
export const queryApplicants = async (req, res) => {
  ...
  query = {
    ...
    ...{
      ...
      ...(approvedApplication
        ? { approvedApplication: Boolean(approvedApplication) }
        : {}),
    },
  };
...
export const approvedApplication = async (req, res) => {
  try {
    ...
    const thisApplicant = await ApplicantService.findByIdAndUpdateApproved(
      _id,
      isApproved
    );
    ...
    if (thisApplicant && isApproved) {
      ...
      return res.send({
        message: "not approved application email has been sent!",
      });
    } else {
...
export const filterApplicants = async (req, res) => {
  ...
  const notReviewed = await Applicant.find({
